...
async def get_user_params(db: AsyncSession, user_id: UUID) -> SRSUserParams:
    ...
    """
    result = await db.execute(select(SRSUserParams).where(SRSUserParams.user_id == user_id))
...
async def update_from_attempt(
    ...
    if telemetry:
        ...
        # Validate telemetry
        time_spent_ms, change_count, warnings = validate_telemetry(time_spent_ms, change_count)
        if warnings:
        ...
        try:
            ...
            result = await db.execute(
                select(SRSConceptState).where(
                    and_(
                        SRSConceptState.user_id == user_id, SRSConceptState.concept_id == concept_id
                    )
                )
            )
...
async def get_user_stats(
    ...
    result = await db.execute(
        select(func.count(SRSConceptState.concept_id)).where(SRSConceptState.user_id == user_id)
    )
