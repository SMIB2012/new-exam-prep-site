...
class SRSUserParams(Base):
    ...
    weights_json: Mapped[dict | None] = mapped_column(JSONB, nullable=True, comment="Personalized FSRS weights (19 params for v6)")
    desired_retention: Mapped[float] = mapped_column(Float, nullable=False, default=0.90, comment="Target retention probability")
    
    # Training metadata
    n_review_logs: Mapped[int] = mapped_column(Integer, nullable=False, default=0, comment="Total review logs for this user")
    last_trained_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True, comment="Last time weights were trained")
...
class SRSConceptState(Base):
    ...
    stability: Mapped[float] = mapped_column(Float, nullable=False, comment="Memory stability (days)")
    difficulty: Mapped[float] = mapped_column(Float, nullable=False, comment="Item difficulty [0, 10]")
    
    # Scheduling
    last_reviewed_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    due_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True, comment="Next review due date")
    last_retrievability: Mapped[float | None] = mapped_column(Float, nullable=True, comment="Retrievability at last review [0, 1]")
...
class SRSReviewLog(Base):
    ...
    rating: Mapped[int] = mapped_column(Integer, nullable=False, comment="FSRS rating 1-4 (Again, Hard, Good, Easy)")
    correct: Mapped[bool] = mapped_column(Boolean, nullable=False, comment="Whether answer was correct")
    delta_days: Mapped[float] = mapped_column(Float, nullable=False, comment="Days since last review")
    
    # Telemetry (optional)
    time_spent_ms: Mapped[int | None] = mapped_column(Integer, nullable=True, comment="Time spent on question (ms)")
    change_count: Mapped[int | None] = mapped_column(Integer, nullable=True, comment="Number of answer changes")
    ...
    # Traceability
    raw_attempt_id: Mapped[UUID | None] = mapped_column(PG_UUID(as_uuid=True), nullable=True, comment="Source session_answer.id")
    session_id: Mapped[UUID | None] = mapped_column(PG_UUID(as_uuid=True), nullable=True, comment="Source test_session.id")
