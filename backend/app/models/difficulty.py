...
class DifficultyUserRating(Base):
    ...
    scope_id: Mapped[UUID | None] = mapped_column(
        PG_UUID(as_uuid=True),
        nullable=True,
        comment="NULL for GLOBAL; theme_id for THEME"
    )
    
    # Elo rating components
    rating: Mapped[float] = mapped_column(Float, nullable=False, default=0.0, comment="User ability (θ)")
    uncertainty: Mapped[float] = mapped_column(Float, nullable=False, comment="Rating deviation (RD-like)")
    ...
    __table_args__ = (
        Index("idx_difficulty_user_rating_lookup", "user_id", "scope_type", "scope_id", unique=True),
...
class DifficultyQuestionRating(Base):
    ...
    # Elo rating components
    rating: Mapped[float] = mapped_column(Float, nullable=False, default=0.0, comment="Question difficulty (b)")
    ...
    __table_args__ = (
        Index("idx_difficulty_question_rating_lookup", "question_id", "scope_type", "scope_id", unique=True),
...
class DifficultyUpdateLog(Base):
    ...
    scope_used: Mapped[str] = mapped_column(
        String(20),
        nullable=False,
        comment="GLOBAL, THEME, or BOTH"
    )
    
    # Observation
    score: Mapped[bool] = mapped_column(Boolean, nullable=False, comment="Correct (true) or incorrect (false)")
    p_pred: Mapped[float] = mapped_column(Float, nullable=False, comment="Predicted probability of correct")
    ...
    k_q_used: Mapped[float] = mapped_column(Float, nullable=False, comment="Question K value applied")
    ...
    run_id: Mapped[UUID | None] = mapped_column(
        PG_UUID(as_uuid=True),
        ForeignKey("algo_runs.id", ondelete="SET NULL"),
        nullable=True
    )
