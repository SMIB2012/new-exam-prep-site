...
def test_invalid_login_generic():
    ...
    try:
        req = Request(f"{BASE_URL}/auth/signup", data=json.dumps(signup_data).encode(), 
    ...
    wrong_pass_status, wrong_pass_response, _ = make_request("POST", "/auth/login", 
                                                              data={"email": test_email, "password": "WrongPass123!"})
    
    # Test 2: Non-existent email
    fake_email_status, fake_email_response, _ = make_request("POST", "/auth/login", 
                                                              data={"email": f"fake_{int(time.time())}@example.com", 
                                                                    "password": "AnyPass123!"})
    ...
    if wrong_pass_status == fake_email_status:
        ...
        if wrong_pass_status in (401, 403) and (wrong_pass_code == fake_email_code or 
                                                 (wrong_pass_code and fake_email_code)):
...
def test_rate_limiting():
    ...
    for i in range(15):  # Should hit rate limit before this
        attempts += 1
        status, response, headers = make_request("POST", "/auth/login", 
                                                 data={"email": test_email, "password": "WrongPass123!"})
        last_status = status
...
def test_account_lockout():
    ...
    try:
        req = Request(f"http://localhost:8000/v1/auth/signup", data=json.dumps(signup_data).encode(), 
                     headers={"Content-Type": "application/json"}, method="POST")
        with urlopen(req, timeout=10):
    ...
    for i in range(12):  # Need at least 9 attempts (8 failures + 1 lockout response)
        ...
        if status == 403:
            ...
            if error_code == "ACCOUNT_LOCKED" or "lock" in error_code.upper():
                ...
                lock_expires_in = response.get("error", {}).get("details", {}).get("lock_expires_in")
