...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('algo_version_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('algo_versions.id', ondelete='SET NULL'), nullable=True),
        sa.Column('params_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('algo_params.id', ondelete='SET NULL'), nullable=True),
        sa.Column('run_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('algo_runs.id', ondelete='SET NULL'), nullable=True),
        sa.Column('created_at', sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()),
    )
    
    # Indexes for difficulty_update_log
    op.create_index(
        'idx_difficulty_update_log_user',
        'difficulty_update_log',
        ['user_id', 'created_at']
    )
    op.create_index(
        'idx_difficulty_update_log_question',
        'difficulty_update_log',
        ['question_id', 'created_at']
    )
    op.create_index(
        'idx_difficulty_update_log_theme',
        'difficulty_update_log',
        ['theme_id', 'created_at']
    )
    op.create_index(
        'idx_difficulty_update_log_attempt',
        'difficulty_update_log',
        ['attempt_id'],
        unique=True
    )
...
def downgrade() -> None:
    ...
    op.drop_index('idx_difficulty_question_rating_distribution', table_name='difficulty_question_rating')
